...
# jbuhacoff reset logging level to INFO
...
# jbuhacoff added extra parameters for decryption 
...
def extract_tarball(fileobj, path, callback=None, extra_params=None):
    ...
    # jbuhacoff replace single tar command with openssl-tar pipeline
    ...
    if extra_params['mh_encrypted'] == 'true':
        ...
        tar_cmd = ["/bin/sh","-c","/usr/bin/openssl enc -d -aes-128-ofb -pass file:%s | tar -zx --directory=%s" % (extra_params['mh_dek_file'],path)]
    else:
    ...
    # jbuhacoff end replace tar command with pipeline for decryption
    ...
    # jbuhacoff replacing finish_subprocess with pipe-friendly version, need to make it a separate def
    finish_subprocess(tar_proc, tar_cmd)
