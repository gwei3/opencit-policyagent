...
def _fetch_image(context, session, instance, name_label, image_id, image_type):
    ...
    """
    # jbuhacoff start
    LOG.debug(_("mhagent hook: vm_utils.py _fetch_image"))
    if 'mh_encrypted' in instance and 'mh_checksum' in instance and 'mh_dek_url' in instance:
        LOG.info(_("mhagent hook into _fetch_image ENCRYPTED"), instance=instance)
        subprocess.call(['/usr/local/bin/mhagent','log','vm_utils.py _fetch_image ENCRYPTED'])
    else:
        LOG.info(_("mhagent hook into _fetch_image PLAIN"), instance=instance)
        subprocess.call(['/usr/local/bin/mhagent','log','vm_utils.py _fetch_image PLAIN'])
    # jbuhacoff end
    if image_type == ImageType.DISK_VHD:
        LOG.info(_("mhagent hook into _fetch_image with decryption for VHD IMAGE"), instance=instance)
        vdis = _fetch_vhd_image(context, session, instance, image_id)
    else:
        LOG.info(_("mhagent hook into _fetch_image with decryption for RAW IMAGE"), instance=instance)
...
def _fetch_using_dom0_plugin_with_retry(context, session, image_id,
    ...
    sleep_time = 0.5
    # jbuhacoff log statement
    LOG.debug(_("mhagent hook: vm_utils.py _fetch_using_dom0_plugin_with_retry"))
...
def _fetch_vhd_image(context, session, instance, image_id):
    ...
    if 'mh_encrypted' in instance:
        LOG.debug(_("mhagent hook: vm_utils.py _fetch_vhd_image mh_dek_url=%s"), instance['mh_dek_url'])
        params['mh_encrypted'] = instance['mh_encrypted']
        params['mh_checksum'] = instance['mh_checksum']
        params['mh_dek_url'] = instance['mh_dek_url']
    else:
        LOG.debug(_("mhagent hook: vm_utils.py _fetch_vhd_image PLAIN"))
    ...
    params['extra_headers'] = { 'X-Auth-Token':  getattr(context, 'auth_token', None) }
    LOG.debug(_("mhagent hook: vm_utils.py _fetch_vhd_image added X-Auth-Token: %s"), params['extra_headers']['X-Auth-Token'])
    # jbuhacoff end

    if (_image_uses_bittorrent(context, instance) and
            _add_torrent_url(instance, image_id, params)):

        LOG.debug(_("mhagent hook: vm_utils.py will download image from bittorrent"))
