...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        # other interesting attributes: instance['host'] is physical hostname, instance['uuid'], instance['hostname'] is VM hostname
        if 'properties' in image_meta and 'mh_encrypted' in image_meta['properties']:
                ...
                # copy mh_encrypted and mh_checksum to the instance data structure so it will be available in _create_image
                # the alternative (or maybe in addition) we can also include this info in the xml generated by to_xml
                # because that xml is passed to _create_image
        ...
        xml = self.to_xml(instance, network_info, image_meta,
                           block_device_info=block_device_info)
        # jbuhacoff start
        subprocess.call(['/usr/local/bin/mhagent','log','--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--comment="after to_xml"'])
        # jbuhacoff end
        ...
        # jbuhacoff start
        subprocess.call(['/usr/local/bin/mhagent','log','--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--comment="after create_image"'])
        # jbuhacoff end
        self._create_domain_and_network(xml, instance, network_info,
                                        block_device_info)
    ...
    def _create_image(self, context, instance, libvirt_xml, suffix='',
        ...
        # jbuhacoff start
        LOG.debug("mhagent hook: driver.py _create_image:  %s", basepath('libvirt.xml'))
        # jbuhacoff end
        ...
        if 'mh_encrypted' in instance:
            if instance['mh_encrypted'] == 'true':
                LOG.debug("mhagent hook: driver.py _create_image: image is encrypted, original checksum %s and root_fname is %s", instance['mh_checksum'], root_fname)
	        subprocess.call(['/usr/local/bin/mhagent','log','--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--comment="after all the defs in _create_image"'])
	        # ### XXX this is too early  subprocess.call(['/usr/local/bin/mhagent','launch','--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--base-image='+root_fname,'--checksum='+instance['mh_checksum'],'--dek-url='+instance['mh_dek_url']])

        else:
            LOG.debug("mhagent hook: driver.py _create_image: image is NOT encrypted")

        # jbuhacoff end

        LOG.debug("mhagent hook: driver.py _create_image: %s %s", disk_images['image_id'], root_fname)
        LOG.debug("mhagent hook: disk_images=%s", jsonutils.dumps(disk_images))
        LOG.debug("mhagent hook: files=%s", jsonutils.dumps(files))
        subprocess.call(['/usr/local/bin/mhagent','log','virt/libvirt/driver.py:_create_image',disk_images['image_id'],root_fname])
        LOG.debug("mhagent hook: driver.py _create_image: check mhagent log")
        # jbuhacoff end
        ...
        # jbuhacoff start
        subprocess.call(['/usr/local/bin/mhagent','log','--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--comment="in _create_image before image disk cache fetch"'])
        # jbuhacoff end

        if not self._volume_in_mapping(self.default_root_device,
                                       block_device_info):
            subprocess.call(['/usr/local/bin/mhagent','log','if not self._volume_in_mapping', '--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--comment="in _create_image after _volume_in_mapping"'])
            image('disk').cache(fetch_func=libvirt_utils.fetch_image,
                                context=context,
                                filename=root_fname,
                                size=size,
                                image_id=disk_images['image_id'],
                                instance=instance, # jbuhacoff added
                                user_id=instance['user_id'],
                                project_id=instance['project_id'])
        else:
            subprocess.call(['/usr/local/bin/mhagent','log','else ** if not self._volume_in_mapping', '--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--comment="in _create_image after _volume_in_mapping"'])
        

        # jbuhacoff start
        subprocess.call(['/usr/local/bin/mhagent','log','--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--comment="in _create_image after _volume_in_mapping"'])
        # calling it here results in a grub error the first launch, and success second launch... probably because some work was already done with the encrypted image which cannot be used yet. 
	# ### XXX subprocess.call(['/usr/local/bin/mhagent','launch','--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--base-image='+root_fname,'--checksum='+instance['mh_checksum'],'--dek-url='+instance['mh_dek_url']])
        # jbuhacoff end
        ...
        # jbuhacoff start
        subprocess.call(['/usr/local/bin/mhagent','log','--project-id='+instance['project_id'],'--instance-name='+instance['name'],'--comment="end of _create_image"'])
        # jbuhacoff end
